...
from .models import *
from .forms import *
import datetime, json, csv, time, os
...
def register_views(request):
	...
	if request.method == 'GET':
		...
		user.upwd = make_password(request.POST.get('upwd'))
		...
		# 內建auth_user存入用戶訊息
		# auth_user = Auth_user()
		# auth_user.username = request.POST.get('uphone')
		# auth_user.password = request.POST.get('upwd')
		# auth_user.email = request.POST.get('uemail')
		# auth_user.save()
		...
		Auth_user.objects.create_user(username, email, password)
...
=======
		# http://3.114.174.92:5000/
		if 'http://localhost:8000/account/password-reset/complete/' in url:
>>>>>>> b3b5c2a57f133598cedabe0826c6e1427d62fedd
			url = '/'
		...
		if form.is_valid():
			...
			# 根據uphone取得使用者數據
			user = User.objects.get(uphone=dic['uphone'])

			print("check_password>>>", check_password(dic['upwd'],user.upwd))

			# 判斷使用者, 密碼是否存在於數據庫
			if check_password(dic['upwd'],user.upwd):
				request.session['uphone'] = dic['uphone']
				request.session['fruit_uid'] = user.id
				# 判斷是否存入Cookie
				# if request.POST.get('isSaved'):
				if 'isSaved' in request.POST:
					resp.set_cookie('fruit_uid', user.id, 60*60*24*7)
...
# 判斷使用者帳號是否使用過
def email_check_views(request):

	# 前端提供querystring用戶登錄名
	uemail = request.GET.get('uemail','not find')
	print('email:',uemail)
	# 確認是否為有人註冊過這個uphone
	users = User.objects.filter(uemail=uemail)
