...
function orders(){
  ...
  $.get('/carts_order/',function(data){
    ...
    // 從obj中取出good和cart, 並轉換為json物件
      ...
      // 當輸入框發生變化時觸發
      // $('input[name="'+cart_json.cart_id+ '"]').change(function(){

      //   var ccount = $('input[name="'+cart_json.cart_id+ '"]').val();
      //   console.log('ccount:',ccount);
          ...
          $.get(url,{'cart_id':modifying_element.attr('name')},function(data_from_backend){
              ...
              var after = before.replace(
                  '<p class="quantity"' + ' name="' + modifying_element.attr('name') + '"' + '>' +
                    '<input class="button" type="button" style="width:20px" value="+" onclick="plusFunc('+ modifying_element.attr('name') +')">' +
                   '<input' + ' name="' + modifying_element.attr('name')+ '"' + ' id="ccount" class="quan_text" type="text" oninput="modifyDetect();" value="' + modifying_element.attr('value') +'" style="width:20px">' +
                   '<input class="button" type="button" style="width:20px" value="-" onclick="minusFunc(' + modifying_element.attr('name') + ')">' +
                   '</p>',
  ...
  $.each(not_modifying_element,function(i,obj){
      ...
      // if($(obj).attr('name')==cart_id & changeNotList.indexOf($(obj).attr('name'))<=-1){
          $.get(url,{'cart_id':cart_id},function(data_from_backend){
            ...
            '<p class="quantity"'+ ' name="'+ cart_id + '"' +'>' + 
            '<input class="button" type="button" style="width:20px" value="+" onclick="plusFunc(' + cart_id + ')">' +
            '<input' + ' name="' + cart_id + '"' + ' id="ccount" class="quan_text" type="text" oninput="modifyDetect();" value="' + data_from_backend.ccount +'" style="width:20px">' +
            '<input class="button" type="button" style="width:20px" value="-" onclick="minusFunc(' + cart_id + ')">' +
            '</p>');
            $('#main').html(after);
          },'json');
